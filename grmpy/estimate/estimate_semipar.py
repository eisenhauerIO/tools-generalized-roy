...
Note: Semiparametric estimation requires the 'kernreg' package which
...
def semipar_fit(dict_, data):
    ...
    Note: Requires the 'kernreg' package (Python <3.9 only).
    ...
    rslt: dict
        ...
        - b0
    """
    if not KERNREG_AVAILABLE:
        raise ImportError(
            "Semiparametric estimation requires the 'kernreg' package, "
            "which only supports Python <3.9. Please use Python 3.8 or earlier, "
            "or install a compatible fork of kernreg."
        )
